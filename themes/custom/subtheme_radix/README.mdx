...
subtheme_radix theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of subtheme_radix theme and run the following commands:
`nvm use`
...
`drush en subtheme_radix -y`
`drush config-set system.theme default subtheme_radix -y`
